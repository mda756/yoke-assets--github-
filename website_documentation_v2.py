...
async def capture_website():
    ...
    async with async_playwright() as p:
        ...
        # Get all navigation links (from header, nav, menu)
        print("\n2. Finding navigation links...")
        ...
        for link in links:
            ...
            # Only internal links
            if (parsed.netloc == home_domain or not parsed.netloc):
                full_url = urljoin(base_url, href)

                # Remove trailing slash for comparison
                clean_url = full_url.rstrip('/')

                if clean_url not in seen_urls and clean_url != base_url.rstrip('/'):
        ...
        print(f"   Found {len(internal_links)} unique navigation links")
        ...
        for i, link in enumerate(internal_links, 1):
            try:
                ...
                print(f"   ERROR: Could not capture {link['url']}: {str(e)[:100]}")
                continue
